    ...
    public class MainDialog : ComponentDialog
    {
            ...
            CancellationToken cancellationToken)
        {
            if (!_luisRecognizer.IsConfigured)
                ...
                return await stepContext.BeginDialogAsync(nameof(FlightBookingDialog), new FlightBookingDetails(),
                    cancellationToken);

            // Call LUIS and gather any potential booking details. (Note the TurnContext has the response to the prompt.)
            var luisResult =
                await _luisRecognizer.RecognizeAsync<LightningTravelBooking>(stepContext.Context, cancellationToken);
            switch (luisResult.TopIntent().intent)
            {
                case LightningTravelBooking.Intent.BookFlight:
                    await ShowWarningForUnsupportedCities(stepContext.Context, luisResult, cancellationToken);

                    // Initialize BookingDetails with any entities we may have found in the response.
                    var flightBookingDetails = new FlightBookingDetails
                    {
                    ...
                    return await stepContext.BeginDialogAsync(nameof(FlightBookingDialog), flightBookingDetails,
                        cancellationToken);

                case LightningTravelBooking.Intent.BookHotel:
        ...
        private static async Task ShowWarningForUnsupportedCities(ITurnContext context,
            LightningTravelBooking luisResult, CancellationToken cancellationToken)
        {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (stepContext.Result is FlightBookingDetails result)
            {
